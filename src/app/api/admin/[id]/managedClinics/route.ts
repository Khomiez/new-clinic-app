...
export async function GET(
  ...
  try {
    ...
    // Ensure managedClinics is always an array, even if it's null/undefined
    ...
    return NextResponse.json(
      { success: false, error: "Failed to fetch managed clinics" },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const alreadyLinked = admin.managedClinics.some(
      (cid: Types.ObjectId) => cid.toString() === clinicId
    );
    if (alreadyLinked) {
      return NextResponse.json(
        { success: false, error: "Clinic is already managed by this admin" },
        { status: 409 }
      );
    }
    ...
    await Admin.findByIdAndUpdate(id, {
      $addToSet: { managedClinics: clinicId },
    });
    ...
    return NextResponse.json(
      { success: false, error: "Failed to add clinic to admin" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Failed to remove clinic from admin" },
      { status: 500 }
    );
  }
}